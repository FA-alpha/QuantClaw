...
| 用户说法 | 提取回测ID |
|---------|-----------|
| "查看回测 12345 的详情" | `12345` |
| "回测ID abc123 的详细数据" | `abc123` |
| "看一下回测记录 xyz789" | `xyz789` |
| "回测 456 的收益曲线" | `456` |

**注意**：
- 回测ID支持数字和字符串格式
- 如果用户未提供ID，询问："请提供回测记录ID"
- 如果ID不存在，提示："未找到回测记录 {back_id}，请确认ID是否正确"

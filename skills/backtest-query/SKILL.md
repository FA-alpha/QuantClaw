...
**场景**：用户要求某个特定策略类型的"比较好的"策略

**示例**：
- "BTC做多风霆v4策略比较好的有哪些"
- "给我推荐几个ETH网格策略表现最好的"

**处理策略**：需要尝试**多个参数组合**来覆盖不同维度的"好"
...
# 从结果中找到对应的 strategy_type 和 version
# 例如：风霆v4 → strategy_type=11, version=4.0
```
...
2. **分组**：按年份、排序指标分组展示
3. **综合评分**：`sharpe_rate * 0.4 + year_rate * 0.3 - max_loss * 0.3`
4. **推荐**：给出不同风险偏好的推荐理由

...
**规则：先查后问。不管用户说没说币种，都先查列表。**

1. 执行 `query.py --list-coins --agent-id xxx` 获取真实币种数据
2. 用户说了币种 → 在返回数据中匹配（支持名称/简称模糊匹配）
   - 匹配成功 → 继续
   - 匹配失败 → 告知"未找到币种 XXX"，展示可用币种列表并询问
3. 用户未提币种 → 展示可用币种列表并询问

询问话术：
```
请问您想查询哪些币种的策略？以上是当前可用的币种列表。
```

#### 策略类型确认（必需）

**规则：先查后问。不管用户说没说策略类型，都先查列表。**

1. 执行 `query.py --list-strategies --agent-id xxx` 获取真实策略类型数据（含版本名/杠杆/复投等字段）
2. 在返回数据中匹配用户需求：
   - 在策略名称 + 版本名称中搜索关键词（如"风霆"、"3倍"、"复投80"、"杠杆"等）
   - 版本名如 `V4.3-3倍-复投80` 可直接匹配到对应 strategy_type + version
   - 匹配成功 → 继续
   - 匹配失败 → 展示可用策略类型列表并询问
3. 完全无匹配关键词 → 展示列表并询问
...
询问话术：
```
请问您想查询哪种类型的策略？以上是当前可用的策略类型列表。
```

#### ❌ 错误示例（不要这样做）

用户："帮我挑选 doge、bch 的 ai回测策略"
- ❌ 不查列表直接问用户（应该先查 list-strategies，再尝试匹配"ai回测"）
- ✅ 先查列表 → 匹配失败 → 展示策略类型列表 → 询问

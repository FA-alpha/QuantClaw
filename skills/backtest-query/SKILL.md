...
# 4. Python 脚本读取文件并创建 + 输出数据摘要
exec("python3 query.py --create-group --from-file /tmp/result.json --group-name 'xxx' --agent-id qc-xxx")
# stdout 输出紧凑数据（~200 字符）：
#   ✅ 策略组创建成功: xxx (ID: 91)
#   评分: 70.0 | 收益: 126.5% | 回撤: 42.1% | 夏普: 2.06 | 胜率: 98.46%
#   策略: DOGE-多(146.85%,2.53,31.17%), BCH-多(185.66%,1.62,53.07%), DOGE-空(93.02%,0.92,73.33%), BCH-空(80.46%,3.18,10.85%)

# 5. Agent 解析数据，用自己的语言展示给用户
```

**关键点**：
- **中间查询**：`--quiet` 只输出状态（~50 字符），完整数据（6KB+）不传给 LLM
- **最终展示**：Python 输出紧凑数据（~200 字符），Agent 解析后用自己的语言展示
- **灵活展示**：Agent 可以补充说明、调整格式、突出重点
- **Token 节省**：从 6000+ 字符降到 200 字符（节省 ~97%）
...
**--from-file 输出格式**：
- 自动读取推荐结果文件 → 创建策略组
- 输出紧凑数据（~200 字符）：评分、收益、回撤、夏普、胜率、策略列表
- Agent 解析后用自己的语言展示（可补充说明、调整格式）

...
| 参数 | 说明 | 默认值 | 建议 |
|------|------|--------|------|
| `--max-workers` | 并发线程数 | 10 | 5-20（取决于服务器性能） |
| `--max-qps` | 每秒最大查询数 | 20 | 不超过 API 限流阈值 |
| `--retry-times` | 失败重试次数 | 3 | 网络不稳定时可增加 |

**性能说明**：
- 串行查询：300 组合 × 200ms = **60 秒**
- 并行查询（20 QPS）：300 组合 ÷ 20 = **15 秒** ⚡
- 提速比例：**4-10 倍**（取决于组合数和 QPS）
...
--max-workers 20 \
--max-qps 50
# 组合数: 4×2×16×2×2×5 = 2560 个
# 预计耗时: ~51 秒（并行）vs ~8.5 分钟（串行）
```

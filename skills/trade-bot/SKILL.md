...
**Agent 规则**：
- 任何写操作，必须先运行预览展示结果，用户确认后**原样重跑相同命令**才执行。不得跳过预览。
- **每次查询必须重新运行命令获取最新数据**，严禁复用历史输出。机器人状态/列表/详情等数据随时变化，反复用旧数据会给用户错误信息。
- **操作互斥，不得串台**：如果一个多步操作（如 edit）尚未完成全部步骤（预览→diff→执行），用户中途切到另一个任务（如增加保证金），只执行新任务。上一个未完成的操作不得自动继续。

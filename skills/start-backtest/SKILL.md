...
**强制回复格式（回测成功后必须按此格式回复并立即结束,回复中的xxxx代表具体的参数）：**
```
✅ 回测已提交
   回测ID: xxxx
...
**🔍 当用户要求先去搜索再去回测特定时间的上涨/下跌/震荡行情时，Agent必须使用websearch搜索对应时间段：**
...
**示例场景B：高效处理**
```
用户："回测我这几条策略，币资金占比是XX%,回测时间范围是最近3个月"
...
# ✅ 正确：基于实际年份搜索
web_search("xxxx年加密货币牛市时间段")  

# ❌ 错误：使用假设年份搜索  
web_search("2025年牛市时间段")  # 错误！当前未必是2025年
```

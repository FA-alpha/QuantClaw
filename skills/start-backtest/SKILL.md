...
**🚨 多策略模式和策略组在共享保证金模式下采用相同的分配方案：**

**必须询问的参数：**
1. **各币种做多保证金占比** - 如："BTC做多占40%，DOGE做多占60%"
2. **各币种做空保证金占比** - 如："BTC做空占50%，DOGE做空占50%" 
3. **AI回测时间类型占比** - 如："2025年震荡占70%，2025年牛市占30%"

**询问模板：**
```
您选择了共享保证金模式，请提供详细的分配方案：

📊 **币种做多分配比例：**
- 各币种做多策略的保证金占比？
- 如：BTC做多40%，DOGE做多60%，SOL做多30%

📊 **币种做空分配比例：**  
- 各币种做空策略的保证金占比？
- 如：BTC做空50%，DOGE做空50%，SOL做空20%

📊 **市场行情分配比例：**
- 各AI回测时间类型的占比？
- 如：2025年震荡70%，2025年牛市30%，最近1年40%

如果某个参数您没有特别要求，我会使用平均分配。
```
...
**示例场景1：智能参数检查处理**
```
用户："共享模式，DOGE占60%"  # 其他参数缺失

Agent处理流程：
1. 获取策略组内策略ID：4637,50722,50723
2. 调用参数完整性检查：
   python skills/start-backtest/backtest_monitor.py \
     --check-allocation \
     --strategy-ids "4637,50722,50723" \
...
3. 检查结果显示is_complete: false，missing包含：
   - SOL做空占比缺失
   - BTC做多占比缺失

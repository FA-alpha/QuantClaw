...
**✅ 正确的时间处理：**
- ✅ **涉及时间的请求时，必须先调用 `exec(command: "date")` 获取当前系统时间**
- ✅ **理解用户时间描述前先确认实际日期**（如"今年"、"最近"、"当前"等）
- ✅ **使用准确的年份进行web搜索**，提高搜索结果准确度
- ✅ **基于真实时间解析用户意图**，避免时间理解偏差
...
```bash
# 🚨 重要：先获取当前时间再搜索
exec(command: "date")  # 输出：Mon May 19 03:07:29 UTC 2026

# ✅ 正确：基于实际年份搜索
web_search("2026年加密货币牛市时间段")  # 当前确实是2026年
web_search("2026年5月比特币价格")      # 精确到月份

# ❌ 错误：使用假设年份搜索  
web_search("2025年牛市时间段")  # 错误！当前已经是2026年
```
...
→ 确认：当前是2026年5月  
→ 理解：用户指的是"2026年的牛市时间段"
→ 搜索：web_search("2026年加密货币牛市时间段")

用户："最近的价格走势"  
→ 先执行：exec(command: "date")
→ 确认：当前是2026年5月19日
→ 理解：用户指的是"2026年5月最新价格走势"
→ 搜索：web_search("BTC bitcoin price latest May 2026")
```
